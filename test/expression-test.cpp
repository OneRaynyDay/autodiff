...
TEST_CASE( "et::expression can be initialized.", "[et::expression::expression]") {
    ...
    REQUIRE(exp.getRoot().getValue<double>() == 10);

    SECTION( "et::expression's root will have correct children." ) {
        et::var a(10), b(5), c(15), d(2);
        et::var root = (a + b) + (c + d);
        et::expression exp(root);
        REQUIRE(exp.getRoot().getChildren()[0].getChildren()[0].getValue<double>() == 10);
        REQUIRE(exp.getRoot().getChildren()[0].getChildren()[1].getValue<double>() == 5);
        REQUIRE(exp.getRoot().getChildren()[1].getChildren()[0].getValue<double>() == 15);
        REQUIRE(exp.getRoot().getChildren()[1].getChildren()[1].getValue<double>() == 2);
    }
}
//
